...
def available_rides():
    ...
    cur.execute("SELECT rides.id, passenger_id, status FROM rides WHERE status='pending'")
    rides = cur.fetchall()
    cur.close()
    conn.close()
...
def accept_ride(ride_id: int, driver_id: int):
    ...
    ride = cur.fetchone()
    if not ride or ride[0] != 'pending':
        cur.close()
        conn.close()
        logger.warning(f"Ride {ride_id} not available to accept", extra={"trace_id": trace_id, "span_id": span_id})
...
def complete_ride(ride_id: int, driver_id: int):
    ...
    conn = get_db_conn()
    cur = conn.cursor()
    # Verify ride accepted by this driver
    cur.execute("SELECT status, driver_id FROM rides WHERE id=%s", (ride_id,))
    ride = cur.fetchone()
    if not ride or ride[0] != 'accepted' or ride[1] != driver_id:
        cur.close()
        conn.close()
    ...
    # Mark ride completed and driver available
